    ...
    <Layout>
        <div class="row justify-center q-px-md q-py-lg">
            <div class="col-xs-12 col-sm-12 col-md-10 col-lg-9">
                    ...
                    <div class="row items-center q-col-gutter-sm">
                        <div class="col">
                            <div class="row items-center q-gutter-sm">
                                <div class="text-subtitle1 text-weight-medium">{{ accountability.description }}</div>
                                <q-badge :color="statusColor" :label="accountability.status || 'Borrador'"
                                    class="q-pa-xs" />
                            </div>
                            <div class="text-caption text-grey q-mt-xs">
                                {{ accountability.employee_name }} &middot; Rendicion #{{ accountability.id }}
                            </div>
                        </div>
                        <div class="col-auto q-gutter-sm">
                            <q-btn v-if="isEditable" color="positive" icon="send" label="Enviar" size="12px" no-caps
                                unelevated aria-label="Enviar rendicion a autorizacion"
                                @click="HandleUpdateStatus('Pendiente')" />
                            <q-btn color="primary" icon="print" label="Imprimir" size="12px" no-caps flat
                                aria-label="Imprimir reporte de rendicion" @click="HandleReportAccountability()" />
                        </div>
                    </div>
                </q-card>

                <!-- Status Banner -->
                <q-banner v-if="statusMessage" :class="['card-form q-mb-md text-white', 'bg-' + statusColor]" dense
                    rounded>
                ...
                <!-- Informacion General (colapsable) -->
                <q-expansion-item class="card-form q-mb-md overflow-hidden rounded-borders" icon="info"
                    label="Informacion General" header-class="text-subtitle2 text-weight-medium" default-value dense default-opened>
                    <q-card>
                        <q-card-section>
                            <!-- Resumen -->
                ...
                <q-card class="card-form">
                    ...
                    <q-card-section class="q-pb-none">
                        <div class="row items-center q-col-gutter-sm">
                            ...
                            <div class="col-auto">
                                <q-input v-model="filter" outlined dense placeholder="Buscar" name="search-detail"
                                    autocomplete="off" aria-label="Buscar documentos en el detalle" class="input-theme"
                            ...
                            <div class="col-auto" v-if="isEditable">
                                <q-btn color="primary" icon="add" label="Nuevo" size="12px" no-caps unelevated
                                    aria-label="Crear nuevo documento" @click="HandleCreateDocument()" />
                            </div>
                        </div>
                    </q-card-section>
                    ...
                    <q-card-section>
                        ...
                        <div v-if="filteredDocuments.length === 0" class="text-center q-pa-xl">
                            ...
                            <q-btn v-if="!filter && isEditable" color="primary" label="Crear Documento" icon="add"
                                no-caps unelevated class="q-mt-md" @click="HandleCreateDocument()" />
                        </div>

                        <!-- Document Cards -->
                        <div v-else class="q-gutter-md">
                            <q-card v-for="doc,i in filteredDocuments" :key="doc.id" flat bordered class="doc-card">
                                <!-- Card Header: key info always visible -->
                                    ...
                                    <div class="row items-center q-col-gutter-sm">
                                        ...
                                        <div class="col">
                                            ...
                                            <div class="text-caption text-grey q-mt-xs">
                                                ...
                                                <span v-if="doc.document_number"> &middot; Factura #{{
                                                    doc.document_number }}</span>
                                        ...
                                        <div class="col-auto q-gutter-xs">
                                            <q-btn v-if="doc.cuf" flat round dense size="sm" icon="receipt_long"
                                                color="teal" :loading="loadingFactura === doc.id"
                                                aria-label="Ver datos originales de factura SIAT"
                                                @click="HandleConsultaFactura(doc)">
                                                <q-tooltip>Ver Factura SIAT</q-tooltip>
                                            </q-btn>
                                            <q-btn v-if="isEditable" flat round dense size="sm" icon="edit"
                                                color="secondary" aria-label="Editar documento"
                                                @click="HandleEditDocument(doc.id)">
                                                <q-tooltip>Editar</q-tooltip>
                                            </q-btn>
                                            <q-btn v-if="isEditable" flat round dense size="sm" icon="delete_outline"
                                                color="negative" aria-label="Eliminar documento"
                                ...
                                <q-slide-transition>
                                    <div v-show="expandedRows[doc.id]">
                                        ...
                                        <q-card-section class="q-py-sm">
                                            <!-- Libro de Compras -->
                                            <div class="text-caption text-weight-medium text-grey-7 q-mb-sm">Libro de
                                                Compras</div>
                                            <div class="row q-col-gutter-sm q-mb-md">
                                                ...
                                                <div class="col-xs-12 col-sm-8 col-md-6" v-if="doc.cuf">
                                                    ...
                                                    <div class="text-caption" style="word-break: break-all;">{{ doc.cuf
                                                        }}</div>
                                                </div>
                                            ...
                                            <!-- Importes -->
                                            <div class="text-caption text-weight-medium text-grey-7 q-mb-sm">Importes
                                            </div>
                                            <div class="row q-col-gutter-sm q-mb-md">
                                            ...
                                            <div v-if="doc.project_code || doc.distribution_rule_one">
                                                <div class="text-caption text-weight-medium text-grey-7 q-mb-sm">Datos
                                                    Contables</div>
        ...
        <q-dialog v-model="showFacturaDialog">
            <q-card style="width: 700px; max-width: 80vw;">
                <q-bar class="bg-teal q-py-lg">
                    ...
                    <q-space />
                    <q-btn dense flat icon="close" color="white" aria-label="Cerrar detalle de factura"
                ...
                <q-card-section v-if="selectedFactura" class="q-pa-md">
                    <div class="row q-col-gutter-sm q-mb-md">
                        ...
                        <div class="col-6">
                            ...
                            <div class="text-body2">{{ selectedFactura.fechaEmision ?
                                selectedFactura.fechaEmision.substring(0,
                                10) : '-' }}</div>
                        </div>
                        <div class="col-6">
                        ...
                        <div class="col-6">
                            ...
                            <div class="text-body2" style="word-break: break-word;">{{ selectedFactura.razonSocialEmisor
                                }}
                            </div>
                        </div>
                        <div class="col-6" v-if="selectedFactura.direccion">
                            <div class="text-caption text-grey">Direccion</div>
                            <div class="text-body2" style="word-break: break-word;">{{ selectedFactura.direccion }}
                            </div>
                        </div>
                    ...
                    <div v-if="selectedFactura.listaDetalle && selectedFactura.listaDetalle.length > 0">
                        <q-separator class="q-mb-md" />
                        <div class="text-subtitle2 q-mb-sm">Detalle ({{ selectedFactura.listaDetalle.length }} items)
                        </div>
                        <q-virtual-scroll :items="selectedFactura.listaDetalle" :virtual-scroll-item-size="56"
                            style="max-height: 300px;" class="rounded-borders bordered">
                            <template v-slot="{ item, index }">
                                <q-item :key="index" dense>
                                    <q-item-section avatar>
                                        <q-avatar size="24px" color="grey-3" text-color="grey-8" class="text-caption"
                                            style="font-variant-numeric: tabular-nums;">
                                            {{ index + 1 }}
                                        </q-avatar>
                                    </q-item-section>
                                    <q-item-section>
                                        <q-item-label style="word-break: break-word;">{{ item.descripcion
                                            }}</q-item-label>
                ...
                <q-card-actions align="right" class="q-pa-md">
                    <q-btn color="primary" label="Cerrar" no-caps flat size="12px" @click="showFacturaDialog = false" />
                </q-card-actions>
            </q-card>
        </q-dialog>
    </Layout>
...
const isEditable = computed(() => {
    return accountability.status === 'Rechazado' || accountability.status === null;
});
...
async function HandleConsultaFactura(doc) {
    if (!doc.cuf || !doc.nit || !doc.document_number) {
        $q.notify({ type: "negative", message: "Faltan datos para consultar la factura (CUF, NIT o N. Factura)" });
        return;
    }
...
function HandleEditDocument(id) {
    router.visit(
        route("panel.accountability.manage.detail.edit", [
            page.props.profile.id,
            accountability.id,
            id,
        ])
...
function HandleDeleteDocument(id) {
    $q.dialog({
        ...
        router.delete(
            route("panel.accountability.manage.detail.delete", [
                page.props.profile.id,
                accountability.id,
                id,
            ]),
...
function HandleUpdateStatus(status) {
    $q.dialog({
        title: "Confirmar",
        message: "\u00bfEst\u00e1 seguro de enviar a autorizaci\u00f3n?",
        cancel: true,
        persistent: true,
    }).onOk(() => {
        router.post(
            route("panel.accountability.manage.detail.status", [
                page.props.profile.id,
                accountability.id,
            ]),
            { status },
...
function HandleReportAccountability() {
    openURL(
        route("panel.accountability.manage.report", [
            page.props.profile.id,
            accountability.id,
        ])
    );
